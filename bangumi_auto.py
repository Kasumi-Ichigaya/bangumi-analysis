        ...
        for d, name, color, size, line in [
            ...
            if not d.empty:
                ...
                fig.add_scatter(
                    ...
                    customdata=list(zip(d.get("url",[]),d.get("votes",[]),d.get("rank",[]))),
                    marker={**marker,**({"colorbar":cb} if cb else {})},
                    name=name,
                    hovertemplate="<b>%{hovertext}</b><br>标准差: %{x}<br>平均分: %{y}<br>"+("排名: %{customdata[2]}" if name=="有榜单排名" else "Rank: 未上榜")+"<br>投票人数: %{customdata[1]}<extra></extra>"
                )
        # 悬浮高亮点
