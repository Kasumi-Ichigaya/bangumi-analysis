...
- `toggle.js`：负责按钮点击事件  
  - 点击按钮切换显示对应类型图表  
  - 默认显示第一个类型的图表  
- Plotly 库由 Python 插入的第一个图表自动加载，无需手动引入

### 使用方法
1. 将 `template.html` 放在项目目录  
2. 运行 `bangumi_auto.py` 抓取数据  
3. 脚本生成 `index.html`，将图表和按钮插入模板  
4. 打开 `index.html` 交互查看各类型作品评分分布

💡 **提示**：
- 确保 `style.css` 和 `toggle.js` 路径正确  
- 可根据需求自定义 CSS 样式，使图表和按钮更加美观
